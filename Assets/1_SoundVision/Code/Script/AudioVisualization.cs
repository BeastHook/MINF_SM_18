...
public class AudioVisualization : MonoBehaviour
{
    ...
    private GameObject audioReaderObject;
    private AudioSource audioSource;
    private float[] channelOne;
    private float[] channelTwo;
    //private float[] channelThree;

    private void Awake()
    {
        audioReaderObject = GameObject.Find("_AudioReader");
        audioSource = audioReaderObject.GetComponent<AudioSource>();
...
#pragma warning disable CS0618 // Typ oder Element ist veraltet
        channelOne = audioSource.GetOutputData(8192, 0);
#pragma warning restore CS0618 // Typ oder Element ist veraltet
#pragma warning disable CS0618 // Typ oder Element ist veraltet
        channelTwo = audioSource.GetOutputData(8192, 1);
#pragma warning restore CS0618 // Typ oder Element ist veraltet
//#pragma warning disable CS0618 // Typ oder Element ist veraltet
            ...
            case ParticleEffect_Type.startSpeed:
                for (int i = 0; i < audioReader.audioSamples.Length; i++)
                {
                    ...
                    if (audioReader.audioSamples[i] > audioSensibility)
                    {
                        ...
                        //particles[i].transform.position = new Vector3(channelOne[i], channelTwo[i], channelThree[i]);
                        particles[i].transform.position = new Vector3(channelOne[i]*100, channelTwo[i]*100, 1);
                    }
                    else
                    {
                        var main = particles[i].main;
